...
namespace Rtxdi.ReGIR
{
    ...
    public struct ReGIRGridStaticParameters
    {
        ...
        {
            return new ReGIRGridStaticParameters { GridSize = new uint3(16, 16, 16) };
        }
    }
    ...
    public class ReGIRContext
    {
        ...
        public uint GetReGIRLightSlotCount()
        {
            ...
            {
                case ReGIRMode.Grid:  return m_regirGridCalculatedParameters.lightSlotCount;
                case ReGIRMode.Onion: return m_regirOnionCalculatedParameters.lightSlotCount;
                default:              return 0;
            }
        }

        public void SetDynamicParameters(ReGIRDynamicParameters dynamicParameters)
        {
            m_regirDynamicParameters = dynamicParameters;
        }
        ...
        {
            int numLayerGroups = Math.Max(1, Math.Min(ReGIRConstants.RTXDI_ONION_MAX_LAYER_GROUPS,
                (int)staticParams.onionParameters.OnionDetailLayers));
            ...
            for (int layerGroupIndex = 0; layerGroupIndex < numLayerGroups; layerGroupIndex++)
            {
                ...
                for (int ringIndex = 1; ringIndex < layerGroup.ringCount; ringIndex++)
                {
                    ...
                    ring.cellCount    = Math.Max(1, (int)Math.Floor(
                        (float)partitions * (float)Math.Cos(ringIndex * equatorialAngle)));
        ...
        private void ComputeOnionJitterCurve()
        {
            ...
            foreach (var layerGroup in m_regirOnionCalculatedParameters.regirOnionLayers)
            {
                for (int layerIndex = 0; layerIndex < layerGroup.layerCount; layerIndex++)
                {
                    ...
                    for (int ringIndex = 0; ringIndex < layerGroup.ringCount; ringIndex++)
                    {
                        ...
                        float middleElevation = layerGroup.equatorialCellAngle * (float)ringIndex;
                    ...
                    {
                        cubicRootFactors.Add(maxCellRadius * (float)Math.Pow(middleRadius, -1.0 / 3.0));
                    }
                    else
                    {
            ...
            // Median of cubic root factors (outlier-resistant)
            if (cubicRootFactors.Count > 0)
            {
                cubicRootFactors.Sort();
                m_regirOnionCalculatedParameters.regirOnionCubicRootFactor =
                    cubicRootFactors[cubicRootFactors.Count / 2];
            }
            else
            {
            ...
            // Average of linear factors
            float sumLinear = 0f;
            foreach (var f in linearFactors) sumLinear += f;
            m_regirOnionCalculatedParameters.regirOnionLinearFactor =
                sumLinear / Math.Max((float)linearFactors.Count, 1f);
        }
    }
}
